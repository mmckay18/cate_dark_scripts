...
def make_data_txt_file(totality_skydarks_files):
    ...
    totality_skydarks_files = natural_sort(totality_skydarks_files)
    for tif in totality_skydarks_files:
        i+=1
    ...
    t.write('{}_totality_skydarks_statistics.txt'.format(filename),format='ascii.basic', overwrite=True)
    t.write('{}__totality_skydarks_statistics_neat.txt'.format(filename),format='ascii.fixed_width', overwrite=True)
...
def plot_dark_statistics():
    ...
    header_file = glob.glob('*_totality_skydarks_statistics.txt')
    for txt in header_file:
        ...
        plt.grid(color='b')
        plt.title('{} skydarks_frame_averages '.format(filename[0][:11]))
        plt.xlabel('File Number')
        plt.ylabel('Mean (ADU)')
        plt.savefig('{}_skydarks_frame_averages.png'.format(filename[0][:11]))
        plt.show()
        plt.clf()
...
if __name__ == '__main__':
	...
	path=args.path 
	os.chdir(path)
os.system('mkdir totality_skydarks')
os.system('mv *_skydarks*.tif totality_skydarks')
os.system('pwd')
os.chdir('{}totality_skydarks'.format(path))
os.system('rm -r superdark')
os.system('mkdir ../superdark')
os.system('pwd')
i = 1
totality_skydarks_files = glob.glob("*_skydarks*.tif")
make_data_txt_file(totality_skydarks_files)
